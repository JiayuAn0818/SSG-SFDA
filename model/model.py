...
class CVAE(nn.Module):
    ...
    def __init__(self, input_dim=512, y_dim=31, h_dim=400, z_dim=62):
        # 调用父类方法初始化模块的state
        ...
        # 编码器 ： [b, input_dim+label] => [b, z_dim]
        self.fc1 = nn.Linear(input_dim+y_dim, h_dim)  # 第一个全连接层
        self.fc2= nn.Linear(h_dim, z_dim)  # mu
        self.fc3 = nn.Linear(h_dim, z_dim)  # log_var

        # 解码器 ： [b, z_dim+label] => [b, input_dim]
        self.fc4 = nn.Linear(z_dim+y_dim, h_dim)
        self.fc5 = nn.Linear(h_dim, input_dim+y_dim)
    ...
    def to_categrical(self, y: torch.FloatTensor):
        y_n = y.numpy()
        self.lb.fit(list(range(0,self.y_dim)))
    ...
    def reparameterization(self, mu, log_var):
        ...
        return mu + eps.mul(sigma)  # 这里的“*”是点乘的意思
