...
def cal_acc_domain_agnostic(args,loader,model):
    model.eval()
    # d_cls.eval()
    start_test = True
    all_preds, all_label = [], []
...
def train_source(args, dset_loaders):
    ...
    model = VisionTransformer_DomainClassifier(config, args.img_size, zero_head=True, num_classes=args.class_num,
    ...
    while True:
        ...
        for step, batch in enumerate(iter_source):
            ...
            output0 = model.netC(outputs[0])
            output1 = model.netC(outputs[1])
            reg = 0
            count = 0
            for m in masks[0]:
                reg += m.sum()  # numerator
            ...
            reg /= count
            loss = loss_function(output0, y) + loss_function(output1, y) + 0.15 * reg
            ...
            if global_step % 500 == 0 :
                accuracy, _ = cal_acc(args, model, dset_loaders['0ts'],)
                accuracy1, _ = cal_acc(args, model, dset_loaders['1ts'], t=1)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}% | {:.2f}%'.format(
                    0, global_step , t_total, accuracy * 100,
                    accuracy1 * 100)
...
def train_source_g(args, dset_loaders):
    ...
    while True:
        ...
        for step, batch in enumerate(iter_source):
            ...
            loss_g.backward()
            torch.nn.utils.clip_grad_norm(generator.parameters(), 10000)
            optimizer_g.step()
            scheduler_g.step()
            ...
            if global_step % 500 == 0 :
                ...
                generator.eval()
                if args.dset == 'visda':
                    accuracy, class1 = cal_acc(args, model, dset_loaders['0ts'],)
                    accuracy1, class2 = cal_acc(args, model, dset_loaders['1ts'], t=1)
                else:
                    accuracy, _ = cal_acc(args, model, dset_loaders['0ts'],)
                    accuracy1, _ = cal_acc(args, model, dset_loaders['1ts'], t=1)
                loss = vae_test(model, generator, dset_loaders['0ts'])
                if args.dset == 'visda':
                    log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}% | {:.2f}%'.format(
                        0, global_step , t_total, accuracy * 100,
                        accuracy1 * 100)+'\n'+class1+'\n'+class2+ '\n'+'Total-loss = {:.4f}, BCE-Loss = {:.4f}, KLD-loss = {:.4f}'.format(
...
def train_target_near(args, t, model, dset_loaders, netG, ACC_list=None):
    ...
    num_sample = len(dset_loaders['1tr'].dataset)
    feature_bank, score_bank, label_bank, label_bank_raw = init_bank(num_sample, dset_loaders['1tr'], model, args, t)
    few_short_index = []
    if args.shot > 0:
        few_short_index = dset_loaders['1few_short_index']
        few_short_index = torch.from_numpy(few_short_index)
        score_bank[few_short_index] = label_bank[few_short_index]
        unlabel_index = torch.from_numpy(dset_loaders['1unlabel_index'])
    ...
    while global_step <= t_total:
        if global_step % 500 == 0 or global_step == t_total:
            ...
            # print("source")
            if args.dset == 'visda':
                acc1, class1 = cal_acc(args,model, dset_loaders['0ts'], t=0)  # t=0
                acc2, class2 = cal_acc(args,model, dset_loaders['1ts'], t=1)  # t=0
                acc3, class3 = cal_acc_domain_agnostic(args,dset_loaders['0ts'], model)
                acc4, class4 = cal_acc_domain_agnostic(args,dset_loaders['1ts'], model)
            else:
                acc1, _ = cal_acc(args,model, dset_loaders['0ts'], t=0)  # t=0
                acc2, _ = cal_acc(args,model, dset_loaders['1ts'], t=1)  # t=0
                acc3, _ = cal_acc_domain_agnostic(args,dset_loaders['0ts'], model)
                acc4, _ = cal_acc_domain_agnostic(args,dset_loaders['1ts'], model)
            acc_mean = (acc1 + acc2)/2.0
            acc.append(acc1)
            acc.append(acc2)
            acc.append(acc_mean)
            ACC_list.append(acc)
            if args.dset == 'visda':
                log_str = 'Task: {}, Iter:{}/{}; Accuracy on 2 domains = {:.2f}%|{:.2f}%({:.2f}%|{:.2f}%)| Avg.: {:.2f}%.'.format(
                        t, global_step,t_total, acc1 * 100, acc2 * 100,acc3 * 100, acc4 * 100, acc_mean * 100)+ '\n'+ \
                        class1+'\n'+ class2 +'\n'+ class3 +'\n'+ class4
            else:
                log_str = 'Task: {}, Iter:{}/{}; Accuracy on 2 domains = {:.2f}%|{:.2f}%({:.2f}%|{:.2f}%)| Avg.: {:.2f}%.'.format(
                        t, global_step,t_total, acc1 * 100, acc2 * 100,acc3 * 100, acc4 * 100, acc_mean * 100)
        ...
        if args.MixMatch:
            try:
                ...
                batch_iter1 = iter(dset_loaders['1tr_twice_unlabeled'])
                input_X1, input_X2, _, index = batch_iter1.next()
            try:
                input_X, y_labeled, index_labeled = batch_iter2.next()
            except:
                batch_iter2 = iter(dset_loaders['1tr_labeled'])
                input_X, y_labeled, index_labeled = batch_iter2.next()
            ...
            if global_step % (len(dset_loaders['1tr_twice'])-1)==0:
                batch_iter = iter(dset_loaders['1tr_twice'])
        ...
        out, labels, dls_out = model.netD(x,args,netG)
        ...
        if args.MixMatch and flag ==1 :
            ...
            with torch.no_grad():
                ...
                # mask = torch.cat([dls_out, dls_out2], dim=0)
            ...
            # output_mix, masks, dls_output,labels = model.forward_features(mixed_X.cuda(), t=t, s=args.SMAX,netG=netG,istest=False)
            # output_mix, _ = model.forward_features(mixed_X.cuda(), t=t, s=args.SMAX)
            # output_softmax_mix = nn.Softmax(dim=1)(model.fea_classifier(output_mix))
            ...
            loss += MixMatch_loss
        # if args.MixMatch:
        #     loss += (Dclsmix_loss+Dcls_loss)/2
        # else:
        # print(MixMatch_loss,loss)
        loss += Dcls_loss
        ...
        # print(model.head.named_parameters())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--worker', type=int, default=4, help="dataloader's number of workers")
    ...
    current_folder = "./"
    args.output_dir = osp.join(current_folder, 'SS_OUTPUT_mixmodel1', args.dset, args.order)
    args.model_dir = osp.join(args.output_dir, 'model')
    model_path = args.model_dir + '/source_{}.pt'.format(args.max_epoch)
    ...
    if not osp.exists(model_path_final):
        args.out_file = open(osp.join(args.output_dir, args.log_file + '_mode_{}.txt'.format(args.mode)), 'w')
        ...
        for t in range(args.task_num):
            ...
            if t == 0:
                if not osp.exists(model_path):
                    # Train source model
                    model,generator = train_source_g(args, dset_loaders)
                    # exit()
                else:
                    config = CONFIGS[args.model_type]
                    model = VisionTransformer_DomainClassifier(config, args.img_size, zero_head=True, num_classes=args.class_num, 
                    ...
                    # del unused weight
                    for k in ['domain_classifier.2.weight', 'domain_classifier.2.bias']:
                        del weights_dict[k]
